...
const Card = ({ obj }) => {
  ...
  return (
    <div className={urlPath ? styles.news__card_catalog : styles.news__card}>
      <div className={`${styles.card__image} ${styles.news__card_image}`} style={{ backgroundImage: "url(" + obj.imageUrl + ")" }}>
        <div className={styles.news__card_image_circle}>акция</div>
        <Link className={styles.news__card_view} to={`/product/${obj.id}`}>
          смотреть
        </Link>
        <button className={`${styles.card__form_button} ${styles.news__card_button}`} onClick={() => OnClickAddToCart(obj, isLogged)}>
          <i className={`${styles.card__form_button_icon} icon_shopping_basket`}></i>в корзину
        </button>
      </div>
      <div className={`${styles.card__info} ${styles.news__card_info}`}>
        ...
        <div className={styles.card__info_links}>
          <div className={`${styles.card__links_price} ${styles.news__card_links_price}`}>
            <p>{formatNumber(obj.price)} р</p>
            <span className={`${styles.card__links_price_span} ${styles.news__card_links_price_span}`}>{formatNumber(obj.salePrice)} р</span>
          </div>
        </div>
      </div>
    </div>
