...
import { setActiveSort, setActiveShow, setCurrentPage } from "redux/filters/slice";
      ...
      <div className={styles.products__content_sortby} ref={sortRef}>
        ...
        <ul className={sortListModal ? styles.active : ""}>
          {sortList.map((item, index) => (
            <li className={activeSort.value === item.value ? styles.active : ""} key={index} onClick={() => changeSortValue(item)}>
              {item.name}
            </li>
          ))}
        </ul>
      ...
      <div className={styles.products__content_show} ref={showRef}>
        ...
        <ul className={showListModal ? styles.active : ""}>
          {showList.map((item, index) => (
            <li className={Number(activeShow) === Number(item.value) ? styles.active : ""} key={index} onClick={() => changeShowValue(item.value)}>
              {item.value}
            </li>
          ))}
        </ul>
