...
import { SidebarMenuType } from "pages/ProfilePage";
    ...
    <div className={styles.sidebar}>
      <ul>
        {sidebarMenu.map((item: SidebarMenuType, index: number) => (
          <li className={`${activeLink === index ? styles.active : ""} ${item.borderTop ? styles.borderTop : ""}`} key={index} onClick={() => handleClick(index)}>
            <Link to={item.path}>
              {item.icon}
              {item.name}
            </Link>
